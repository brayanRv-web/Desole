...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-gray-800/50 border border-green-700/30 rounded-2xl shadow-xl overflow-hidden">
        <form action="{{ route('admin.productos.store') }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-8" id="productForm">
            @csrf
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="categoria_id" id="categoria_id" required
                            ...
                            @foreach($categorias as $categoria)
                                <option value="{{ $categoria->id }}" {{ old('categoria_id') == $categoria->id ? 'selected' : '' }} class="text-white">
                                    {{ $categoria->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="estado" id="estado" required
                                class="w-full border border-gray-600 bg-gray-700/50 text-white px-4 py-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200">
                            <option value="activo" {{ old('estado', 'activo') == 'activo' ? 'selected' : '' }} class="text-green-400">🟢 Activo - Disponible para venta</option>
                            <option value="inactivo" {{ old('estado') == 'inactivo' ? 'selected' : '' }} class="text-gray-400">⚪ Inactivo - No visible</option>
                            <option value="agotado" {{ old('estado') == 'agotado' ? 'selected' : '' }} class="text-red-400">🔴 Agotado - Visible pero sin stock</option>
                        </select>
                        <p class="text-xs text-gray-400 flex items-center gap-1">
                            <i class="fas fa-lightbulb text-xs"></i>
            ...
            <div class="bg-gray-700/30 rounded-xl p-6 border border-gray-600/50">
                ...
                <div class="space-y-3">
                    <textarea name="descripcion" id="descripcion" rows="4"
                              ...
                              class="w-full border border-gray-600 bg-gray-700/50 text-white px-4 py-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200 resize-none placeholder-gray-500">{{ old('descripcion') }}</textarea>
