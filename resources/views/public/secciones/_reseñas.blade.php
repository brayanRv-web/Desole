    ...
    <div class="container">
                    ...
                    <div class="card-body p-4"> <!-- Reducido de p-5 a p-4 -->
                        <div class="row align-items-center">
                            ...
                            <div class="col-md-4 mb-4 mb-md-0"> <!-- Reducido de col-md-5 a col-md-4 -->
                                <div class="text-center">
                                    <div class="promedio-calificacion mb-3">
                                        <div class="display-4 fw-bold text-warning mb-1">{{ number_format($promedio, 1) }}</div> <!-- Reducido de display-1 a display-4 -->
                                        ...
                                        <p class="text-muted small">Basado en {{ $totalReseñas }} reseñas</p> <!-- Texto más pequeño -->
                                    </div>
                                   ...
                                   <div class="distribucion-calificaciones horizontal">
                                        @for($i = 1; $i <= 5; $i++)
                                            ...
                                            <div class="d-flex align-items-center mb-2 justify-content-center stat-horizontal">
                                                <span class="text-warning me-2 small">{{ $i }}★</span>
                                                <div class="progress flex-grow-1 mx-2">
                                                    <div class="progress-bar" data-width="{{ number_format($percentage, 2) }}"></div>
                                                </div>
                                                <span class="text-muted ms-2 small">{{ $count }}</span>
                                            </div>
                                        @endfor
                                    </div>
                                </div>
                            ...
                            <div class="col-md-8"> <!-- Aumentado de col-md-7 a col-md-8 -->
                                ...
                                <form id="form-reseña" action="{{ route('reseñas.store') }}" method="POST">
                                    @csrf
        ...
        <div class="row mt-5"> <!-- Reducido espacio -->
            <div class="col-12">
                        ...
                        <div class="card reseña-card h-100">
                            <div class="card-body p-3"> <!-- Reducido padding -->
                                ...
                                <div class="d-flex justify-content-between align-items-start mb-2">
                                    <div>
                                        <h6 class="card-title mb-0 text-white">{{ $reseña->nombre }}</h6> <!-- Reducido espacio -->
                                        @if($reseña->email)
                                        <small class="text-muted">{{ $reseña->email }}</small>
                                        @endif
                                    </div>
                                    <small class="text-muted">{{ $reseña->created_at->format('d/m/Y') }}</small>
                                </div>
                                ...
                                <div class="estrellas mb-2"> <!-- Reducido espacio -->
                                    @for($i = 1; $i <= 5; $i++)
                                        ...
                                        @endif
                                    @endfor
                                    <span class="text-muted ms-1">({{ $reseña->calificacion }})</span> <!-- Reducido espacio -->
                                </div>
                                
                                <!-- Comentario -->
                                <p class="reseña-texto mb-2">{{ Str::limit($reseña->comentario, 100) }}</p> <!-- Limitar texto -->
                ...
                <a href="{{ route('reseñas.publicas') }}" class="btn btn-outline-warning btn-sm">
