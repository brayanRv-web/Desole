	...
	<div class="mb-8 flex items-center justify-between">
		<div>
			<h1 class="text-4xl font-bold text-white mb-2">
				<i class="fas fa-shopping-bag text-green-500 mr-2"></i> Pedido #<?php echo e($pedido->id); ?>
		...
		<div class="px-6 py-3 bg-green-900/30 border border-green-600 rounded-lg">
			...
			<p class="text-xl font-bold text-green-400 capitalize"><?php echo e(str_replace('_', ' ', $pedido->status)); ?></p>
		</div>
	</div>
			...
			<div class="space-y-5">
				...
				<div class="bg-gray-800/50 rounded-lg p-4 border-l-4 border-amber-500">
					...
					<p class="text-white font-bold text-lg mt-2">
						...
						<?php echo e($pedido->direccion ?? 'No disponible'); ?>
		...
		<div class="bg-gray-900 border border-gray-700 rounded-lg p-6 shadow-lg hover:border-green-600 transition">
			...
			<div class="space-y-4">
				<div class="flex justify-between items-center pb-3 border-b border-gray-700">
					...
					<span class="text-white font-bold text-lg"><?php echo e(count($pedido->items ?? [])); ?></span>
				</div>
				<div class="flex justify-between items-center pb-3 border-b border-gray-700">
					...
					<tbody>
						<?php $__currentLoopData = $items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
							<tr class="border-b border-gray-700 hover:bg-gray-800 transition">
								<td class="px-4 py-3 text-gray-400"><?php echo e($item['producto_id'] ?? '-'); ?></td>
								<td class="px-4 py-3 text-white font-semibold">
									<?php echo e($item['nombre'] ?? ($productos[$item['producto_id']]->nombre ?? 'Desconocido')); ?>

								</td>
								<td class="px-4 py-3 text-center text-white">
									<span class="bg-green-900/40 px-3 py-1 rounded-full text-green-400 font-bold">
										<?php echo e($item['cantidad'] ?? 0); ?>

									</span>
								</td>
								<td class="px-4 py-3 text-right text-white">$<?php echo e(number_format($item['precio'] ?? 0, 2)); ?></td>
								<td class="px-4 py-3 text-right text-green-400 font-bold">
									$<?php echo e(number_format(($item['precio'] ?? 0) * ($item['cantidad'] ?? 0), 2)); ?>
	...
	<div class="bg-gray-900 border border-gray-700 rounded-lg p-6 shadow-lg">
		...
		<div class="space-y-3">
			<?php if($pedido->status === 'pendiente'): ?>
				<form method="POST" action="<?php echo e(route('admin.pedidos.updateEstado', $pedido)); ?>" class="inline">
					<?php echo csrf_field(); ?>
...
<?php echo $__env->make('admin.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\Desole\resources\views/admin/pedidos/show.blade.php ENDPATH**/ ?>